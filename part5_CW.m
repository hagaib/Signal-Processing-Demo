...
A = 10.^(SNRin/20);
w = 1.0; %kHz
k = [10 , 10.5]; % bin number (10.5 for in between bins)
...
N_rect_on = floor(4*10.^(0.1*(SNRout-SNRin+PLdb(1))));
N_rect_mid = floor(4*10.^(0.1*(SNRout-SNRin+WCPL(1))));
N_kaiser_on = floor(4*10.^(0.1*(SNRout-SNRin+PLdb(2))));
N_kaiser_mid = floor(4*10.^(0.1*(SNRout-SNRin+WCPL(2))));
...
for i=1:length(SNRin)
    signal = Signal([1 , 1]);
    signal.addComp([2 , A(i) , w]);
    signal.generate(fs(i) , k(1)*w);
    ...
    snr_bin = 10*log10((A(i)/2)^2*N(i) / PL(1));
...
for i=1:length(SNRin)
    signal = Signal([1 , 1]);
    signal.addComp([2 , A(i) , w]);
    signal.generate(fs(i) , k(2)*w);
    win = wingen.generate(1, N(i)+1 , 'dfteven');
    plot((0:N(i)-1)*fs(i)/N(i), abs(fft(win.*signal.xx)));
    title(['CW signal in noise ,  SNR = ', num2str(SNRin(i)) ,  ' , N = ' , num2str(N(i))]);
    snr_bin = 10*log10((A(i)/2)^2*N(i)) - WCPL(1);
    disp(['Amp SNR:' , num2str(SNRin(i)) , ' , calculated SNR in bin:' , num2str(snr_bin) , ' , N = ' , num2str(N(i))]);
end
...
for i=1:length(SNRin)
    ...
    plot((0:N(i)-1)*fs(i)/N(i), abs(fft(win.*signal.xx)));
    title(['CW signal in noise ,  SNR = ', num2str(SNRin(i)) ,  ' , N = ' , num2str(N(i))]);
    snr_bin = 10*log10((A(i)/2)^2*N(i) / PL(2));
    disp(['Amp SNR:' , num2str(SNRin(i)) , ' , calculated SNR in bin:' , num2str(snr_bin) , ' , N = ' , num2str(N(i))]);
end
...
for i=1:length(SNRin)
    ...
    plot((0:N(i)-1)*fs(i)/N(i), abs(fft(win.*signal.xx)));
    title(['CW signal in noise ,  SNR = ', num2str(SNRin(i)) ,  ' , N = ' , num2str(N(i))]);
    snr_bin = 10*log10((A(i)/2)^2*N(i)) - WCPL(2);
    disp(['Amp SNR:' , num2str(SNRin(i)) , ' , calculated SNR in bin:' , num2str(snr_bin) , ' , N = ' , num2str(N(i))]);
end
